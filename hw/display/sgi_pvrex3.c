/*
 * SGI PVRex3 — Paravirtual Newport REX3 graphics for IP54
 *
 * A Newport-compatible REX3 register interface in the PV device bank.
 * Provides the same drawing engine, DCB bus (VC2/XMAP/CMAP/RAMDAC),
 * and VRAM as the real Newport, but attached via sysbus at 0x1F490000
 * instead of a GIO slot. The IRIX kernel pvfb driver registers this
 * as a NEWPORT board via GfxRegisterBoard(), so Xsgi's Newport DDX
 * can drive it directly through user-mapped REX3 registers.
 *
 * Based on sgi_newport.c — all REX3 drawing logic is identical.
 *
 ...
 */
static uint32_t pvrex3_twos_to_sm(uint32_t data, int nbits)
{
 ...
 */
static uint32_t pvrex3_convert_host_to_draw(uint32_t color,
                                              uint8_t hostdepth,
                                              uint8_t drawdepth)
{
 ...
 */
static uint32_t pvrex3_convert_draw_to_host(uint32_t color,
                                              uint8_t drawdepth,
                                              uint8_t hostdepth)
{
 ...
 */
static uint32_t pvrex3_get_default_color(SGIPVRex3State *s)
{
    uint32_t color;
 ...
 */
static uint32_t pvrex3_get_host_color(SGIPVRex3State *s)
{
     ...
     * MAME ref: get_host_color() at newport.cpp:2202-2244
     */
    color = pvrex3_convert_host_to_draw(color, s->dm1_hostdepth,
                                          s->dm1_drawdepth);
 ...
 */
static uint32_t pvrex3_get_rgb_color(SGIPVRex3State *s)
{
    ...
    if (!s->dm1_rgbmode) {
        ...
        return pvrex3_get_default_color(s);
    }
 ...
 */
static int32_t pvrex3_sm_to_signed(int32_t val, int nbits)
{
...
static void pvrex3_iterate_shade(SGIPVRex3State *s)
{
    if (s->slope_red & 0x7fffff) {
        s->curr_color_red += pvrex3_sm_to_signed(s->slope_red, 24);
    }
    if (s->slope_green & 0x7ffff) {
        s->curr_color_green += pvrex3_sm_to_signed(s->slope_green, 20);
    }
    if (s->slope_blue & 0x7ffff) {
        s->curr_color_blue += pvrex3_sm_to_signed(s->slope_blue, 20);
    }
    if (s->slope_alpha & 0x7ffff) {
        s->curr_color_alpha += pvrex3_sm_to_signed(s->slope_alpha, 20);
    }
 ...
 */
static void pvrex3_do_setup(SGIPVRex3State *s)
{
 ...
 */
static void pvrex3_reset_curr_colors(SGIPVRex3State *s)
{
 ...
 */
static void pvrex3_logic_pixel(SGIPVRex3State *s, uint32_t addr,
                                uint32_t src)
{
    uint32_t *buf;
    ...
    if (addr >= (uint32_t)(PVREX3_VRAM_W * PVREX3_VRAM_H)) {
        return;
    }
 ...
 */
static bool pvrex3_pixel_clip_pass(SGIPVRex3State *s, int16_t x, int16_t y)
{
 ...
 */
static void pvrex3_output_pixel(SGIPVRex3State *s, int16_t x, int16_t y,
                                 uint32_t color)
{
    int wx, wy;
    uint32_t addr;
    ...
    if (s->clip_mode && !pvrex3_pixel_clip_pass(s, x, y)) {
        return;
    }
    ...
    if (wx < 0 || wx >= PVREX3_VRAM_W || wy < 0 || wy >= PVREX3_VRAM_H) {
        return;
    }

    addr = (uint32_t)wy * PVREX3_VRAM_W + (uint32_t)wx;
    pvrex3_logic_pixel(s, addr, color);
}
 ...
 */
static void pvrex3_write_x_start(SGIPVRex3State *s, int32_t val)
{
 ...
 */
static void pvrex3_write_y_start(SGIPVRex3State *s, int32_t val)
{
 ...
 */
static void pvrex3_write_x_end(SGIPVRex3State *s, int32_t val)
{
 ...
 */
static void pvrex3_write_y_end(SGIPVRex3State *s, int32_t val)
{
 ...
 */
static void pvrex3_draw_block(SGIPVRex3State *s)
{
    ...
    trace_sgi_pvrex3_draw_block(start_x, start_y, end_x, end_y,
                                s->drawmode0, s->drawmode1);
    color = pvrex3_get_default_color(s);
    ...
    do {
        ...
        do {
            ...
            if (shade) {
                pvrex3_iterate_shade(s);
            }
            if (pattern & (1U << pat_bit)) {
                if (s->dm0_colorhost) {
                    pvrex3_output_pixel(s, sx, sy,
                                         pvrex3_get_host_color(s));
                } else if ((shade || s->dm1_rgbmode) && !s->dm1_fastclear) {
                    pvrex3_output_pixel(s, sx, sy,
                                         pvrex3_get_rgb_color(s));
                } else {
                    pvrex3_output_pixel(s, sx, sy, color);
                }
            } else if (s->dm0_opaque) {
                pvrex3_output_pixel(s, sx, sy, s->color_back);
            }
    ...
    s->iter_x = sx;
    s->iter_y = sy;
    pvrex3_write_x_start(s, (int32_t)sx << 11);
    pvrex3_write_y_start(s, (int32_t)sy << 11);
}
 ...
 */
static void pvrex3_draw_span(SGIPVRex3State *s)
{
    int16_t start_x = s->x_start_int;
    int16_t end_x = s->x_end_int;
    ...
    trace_sgi_pvrex3_draw_span(start_x, y, end_x, s->drawmode0);
    color = pvrex3_get_default_color(s);
    ...
    do {
        ...
        if (pattern & (1U << pat_bit)) {
            if (s->dm0_colorhost) {
                pvrex3_output_pixel(s, sx, y, pvrex3_get_host_color(s));
            } else if ((shade || s->dm1_rgbmode) && !s->dm1_fastclear) {
                pvrex3_output_pixel(s, sx, y, pvrex3_get_rgb_color(s));
            } else {
                pvrex3_output_pixel(s, sx, y, color);
            }
        } else if (s->dm0_opaque) {
            pvrex3_output_pixel(s, sx, y, s->color_back);
        }
        pat_bit = (pat_bit - 1) & 0x1f;
        sx += dx;
...
static void pvrex3_draw_iline(SGIPVRex3State *s)
{
    ...
    for (;;) {
        ...
        if (!skip) {
            ...
            if (s->dm0_colorhost) {
                pvrex3_output_pixel(s, x0, y0, pvrex3_get_host_color(s));
            } else if (shade && !s->dm1_fastclear) {
                pvrex3_output_pixel(s, x0, y0, pvrex3_get_rgb_color(s));
            } else {
                pvrex3_output_pixel(s, x0, y0, color);
            }
        }
        first = false;
        if (is_last) {
    ...
    s->iter_x = x0;
    s->iter_y = y0;
    pvrex3_write_x_start(s, (int32_t)x0 << 11);
    pvrex3_write_y_start(s, (int32_t)y0 << 11);
}
 ...
 */
static void pvrex3_draw_fline(SGIPVRex3State *s)
{
 ...
 */
static void pvrex3_draw_scr2scr(SGIPVRex3State *s)
{
    int16_t start_x = s->x_start_int;
    int16_t start_y = s->y_start_int;
    int16_t end_x = s->x_end_int;
    ...
    trace_sgi_pvrex3_draw_scr2scr(start_x, start_y, end_x, end_y,
                                  move_x, move_y);
    ...
    do {
        do {
            ...
            if (src_wx >= 0 && src_wx < PVREX3_VRAM_W &&
                src_wy >= 0 && src_wy < PVREX3_VRAM_H) {
                src_addr = (uint32_t)src_wy * PVREX3_VRAM_W +
                           (uint32_t)src_wx;
             ...
             */
            pvrex3_output_pixel(s, start_x + move_x, start_y + move_y,
                                 pixel);
    ...
    pvrex3_write_x_start(s, (int32_t)start_x << 11);
    pvrex3_write_y_start(s, (int32_t)start_y << 11);
}
 ...
 */
static uint32_t pvrex3_read_one_pixel(SGIPVRex3State *s)
{
    int wx, wy;
    uint32_t ret = 0;
    int16_t dy;
    ...
    if (wx >= 0 && wx < PVREX3_VRAM_W && wy >= 0 && wy < PVREX3_VRAM_H) {
        uint32_t addr = (uint32_t)wy * PVREX3_VRAM_W + (uint32_t)wx;
        if (s->dm1_planes == 4 || s->dm1_planes == 5) {
    ...
    if (s->dm1_planes == 1 || s->dm1_planes == 2) {
        ret = pvrex3_convert_draw_to_host(ret, s->dm1_drawdepth,
                                            s->dm1_hostdepth);
    }
    ...
    pvrex3_write_x_start(s, (int32_t)s->x_start_int << 11);
    pvrex3_write_y_start(s, (int32_t)s->y_start_int << 11);
 ...
 */
static void newport_do_pixel_read(SGIPVRex3State *s)
{
    ...
    case 0: /* 4bpp */
        ...
        shift = 60;
        for (i = 0; i < width; i++) {
            ret |= (uint64_t)(pvrex3_read_one_pixel(s) & 0x0f) << shift;
            shift -= 4;
        }
        break;
    ...
    case 1: /* 8bpp */
        ...
        shift = 56;
        for (i = 0; i < width; i++) {
            ret |= (uint64_t)(pvrex3_read_one_pixel(s) & 0xff) << shift;
            shift -= 8;
        }
        break;
    ...
    case 2: /* 12bpp */
        ...
        shift = 48;
        for (i = 0; i < width; i++) {
            ret |= (uint64_t)(pvrex3_read_one_pixel(s) & 0xfff) << shift;
            shift -= 16;
        }
        break;
    ...
    case 3: /* 32bpp */
        ...
        shift = 32;
        for (i = 0; i < width; i++) {
            ret |= (uint64_t)pvrex3_read_one_pixel(s) << shift;
            shift -= 32;
        }
        break;
    }
 ...
 */
static void newport_do_rex3_command(SGIPVRex3State *s)
{
 ...
 */
static void pvrex3_dcb_timeout(void *opaque)
{
 ...
 */
static void pvrex3_vrint_deassert(void *opaque)
{
    SGIPVRex3State *s = SGI_PVREX3(opaque);
    s->vrint_active = false;
    /* IRQ line not used — see pvrex3_vblank_timer comment */
}
 ...
 */
static void pvrex3_vblank_timer(void *opaque)
{
    SGIPVRex3State *s = SGI_PVREX3(opaque);
    ...
    if ((s->vc2_reg[VC2_DC_CONTROL] & VC2_DC_ENA_VINTR) &&
        ...
        s->vrint_active = true;
        /*
         * Don't raise the actual IRQ line — the IP54 kernel has no
         * interrupt handler for HEART ISR bit 21 (VRINT). Xsgi polls
         * REX3_STATUS for the VRINT bit instead. If we raise the IRQ,
         * unhandled 60Hz interrupts hang the system.
         */

        /*
         * Schedule status bit deassert after ~500us (VBLANK duration).
         */
 ...
 */
static uint32_t newport_dcb_read(SGIPVRex3State *s)
{
    ...
    case DCB_ADDR_RAMDAC:
        /*
         * BT445 RAMDAC register read.
         * CRS 0: Address register (returns last written address)
         * CRS 3: ID register → 0x4a (BT445 chip ID)
         * CRS 6: GENIO (General IO) → monitor sense pins
         *         0x07 = all sense bits high = multi-sync monitor
         *         This tells the Newport DDX which display modes are valid.
         */
        switch (reg) {
        case 3:
            val = 0x4a;  /* BT445 ID */
            break;
        case 6:
            val = 0x07;  /* GENIO: multi-sync monitor (all sense high) */
            break;
        default:
            val = 0;
            break;
 ...
 */
static uint32_t pvrex3_rgb_unpack(uint32_t pixel, uint8_t pix_size,
                                   uint32_t mode_entry)
{
 ...
 */
static void newport_dcb_write(SGIPVRex3State *s, uint32_t val)
{
    uint32_t slave = (s->dcb_mode >> DCB_MODE_CSADDR_SHIFT) & 0xf;
    uint32_t reg = (s->dcb_mode >> DCB_MODE_REGSEL_SHIFT) & 0x7;
    ...
    trace_sgi_pvrex3_dcb_write(slave, reg, dw, val);
 ...
 */
static void pvrex3_decode_drawmode1(SGIPVRex3State *s)
{
 ...
 */
static void pvrex3_decode_drawmode0(SGIPVRex3State *s)
{
...
static uint64_t sgi_pvrex3_read(void *opaque, hwaddr addr, unsigned size)
{
    SGIPVRex3State *s = SGI_PVREX3(opaque);
    uint32_t val = 0;
    bool is_go;
    ...
    case REX3_STATUS_ALIAS:
         ...
         */
        /*
         * VRINT toggling for DDX compatibility.
         *
         * The Newport DDX polls STATUS waiting for VRINT to be set,
         * then clears it by reading STATUS again. With virtual time,
         * the 60Hz timer fires too slowly during tight polling.
         *
         * Solution: when VINTR is enabled and VRINT is not currently
         * set, set it immediately so the next read sees it. This gives
         * the DDX one VRINT per two reads (set → read → clear → set).
         */
        if ((s->vc2_reg[VC2_DC_CONTROL] & VC2_DC_ENA_VINTR) &&
            !(s->status & REX3_STATUS_VRINT)) {
            s->status |= REX3_STATUS_VRINT;
...
static void sgi_pvrex3_write(void *opaque, hwaddr addr, uint64_t val,
                               unsigned size)
{
    SGIPVRex3State *s = SGI_PVREX3(opaque);
    bool is_go;
    ...
    trace_sgi_pvrex3_reg_write((uint32_t)reg, (int)is_go, (uint32_t)val,
                               size);
    ...
    case REX3_DRAWMODE0:
        s->drawmode0 = val;
        pvrex3_decode_drawmode0(s);
        break;
    case REX3_DRAWMODE1:
        s->drawmode1 = val;
        pvrex3_decode_drawmode1(s);
    ...
    case REX3_XSTART:
        pvrex3_write_x_start(s, (int32_t)val);
        s->x_save_int = s->x_start_int;
        break;
    case REX3_YSTART:
        pvrex3_write_y_start(s, (int32_t)val);
        break;
    case REX3_XEND:
        pvrex3_write_x_end(s, (int32_t)val);
        break;
    case REX3_YEND:
        pvrex3_write_y_end(s, (int32_t)val);
        break;
    case REX3_XSAVE:
    ...
    case REX3_XSTARTF:
        s->x_start_f = val;
        pvrex3_write_x_start(s, (int32_t)val & 0x007fff80);
        s->x_save_int = s->x_start_int;
        break;
    case REX3_YSTARTF:
        s->y_start_f = val;
        pvrex3_write_y_start(s, (int32_t)val & 0x007fff80);
        break;
    case REX3_XENDF:
        s->x_end_f = val;
        pvrex3_write_x_end(s, (int32_t)val & 0x007fff80);
        break;
    case REX3_YENDF:
        s->y_end_f = val;
        pvrex3_write_y_end(s, (int32_t)val & 0x007fff80);
        break;
    case REX3_XSTARTI:
        pvrex3_write_x_start(s, ((int32_t)(int16_t)val) << 11);
        s->x_save_int = s->x_start_int;
        break;
    case REX3_XYSTARTI:
        s->xy_start_i = val;
        pvrex3_write_x_start(s, ((int32_t)(int16_t)(val >> 16)) << 11);
        pvrex3_write_y_start(s, ((int32_t)(int16_t)(val & 0xffff)) << 11);
        s->x_save_int = s->x_start_int;
        break;
    case REX3_XYENDI:
        s->xy_end_i = val;
        pvrex3_write_x_end(s, ((int32_t)(int16_t)(val >> 16)) << 11);
        pvrex3_write_y_end(s, ((int32_t)(int16_t)(val & 0xffff)) << 11);
        break;
    case REX3_XSTARTENDI:
        s->x_start_end_i = val;
        pvrex3_write_x_start(s, ((int32_t)(int16_t)(val >> 16)) << 11);
        pvrex3_write_x_end(s, ((int32_t)(int16_t)(val & 0xffff)) << 11);
        s->x_save_int = s->x_start_int;
        break;
    ...
    case REX3_COLORBLUE:
        ...
        break;
    case REX3_SLOPERED:
        s->slope_red = pvrex3_twos_to_sm((uint32_t)val, 24);
        break;
    case REX3_SLOPEALPHA:
        s->slope_alpha = pvrex3_twos_to_sm((uint32_t)val, 20);
        break;
    case REX3_SLOPEGREEN:
        s->slope_green = pvrex3_twos_to_sm((uint32_t)val, 20);
        break;
    case REX3_SLOPEBLUE:
        s->slope_blue = pvrex3_twos_to_sm((uint32_t)val, 20);
        break;
    case REX3_WRITEMASK:
...
static const MemoryRegionOps sgi_pvrex3_ops = {
    .read = sgi_pvrex3_read,
    .write = sgi_pvrex3_write,
 ...
 * running the same DID/XMAP/CMAP/RAMDAC logic as pvrex3_update_display().
 * Useful for inspecting what's actually in VRAM when the display shows black.
 */
static void pvrex3_dump_vram_ppm(SGIPVRex3State *s, const char *path)
{
    FILE *f;
    int x, y;
    ...
    fprintf(f, "P6\n%d %d\n255\n", PVREX3_SCREEN_W, PVREX3_SCREEN_H);

    /*
     * Render into a pixel buffer (rgb_to_pixel32 form, matching the live
     * display surface) so the hardware cursor can be composited on top before
     * the PPM is written.  Without this, screendumps never show the pointer.
     */
    uint32_t *fb = g_new0(uint32_t, PVREX3_SCREEN_W * PVREX3_SCREEN_H);
    ...
    /*
     * Auto-detect CMAP bank with palette data.
     * The DDX may program its palette into any 256-entry bank (e.g. CMAP1
     * at entries 4609+) while the XMAP mode table entry selects CMAP0.
     * When the XMAP-derived ci_msb points to an empty bank, fall back to
     * the first populated 256-entry bank in the CMAP array.
     */
    uint16_t auto_ci_msb = 0;
    {
        for (int bank = 0; bank < 8192; bank += 256) {
            int nz = 0;
            for (int j = 0; j < 256; j++) {
                if (s->cmap0_palette[bank + j]) {
                    nz++;
                }
            }
            if (nz > 10) {
    ...
    for (y = 0; y < PVREX3_SCREEN_H; y++) {
        const uint32_t *src_rgbci = &s->vram_rgbci[y * PVREX3_VRAM_W];
        const uint32_t *src_cidaux = &s->vram_cidaux[y * PVREX3_VRAM_W];
        uint8_t pix_mode = 0;
        uint8_t pix_size = 1;
        uint16_t ci_msb = 0;
        uint16_t aux_msb = 0;
        ...
        for (x = 0; x < PVREX3_SCREEN_W; x++) {
            ...
            if (pix_mode == 0) {
                ...
                /*
                 * Use XMAP-derived ci_msb if the mode table was programmed,
                 * otherwise fall back to auto-detected CMAP bank.
                 */
                eff_ci_msb = (mode_entry != 0) ? ci_msb : auto_ci_msb;
                rgb = s->cmap0_palette[(eff_ci_msb | ci) & 0x1fff];
            } else {
                rgb = pvrex3_rgb_unpack(pixel, pix_size, mode_entry);
            }
            }
            ...
            fb[y * PVREX3_SCREEN_W + x] = rgb_to_pixel32(r, g, b);
        }
    }
    ...
    for (int i = 0; i < PVREX3_SCREEN_W * PVREX3_SCREEN_H; i++) {
        uint32_t p = fb[i];
        fputc((p >> 16) & 0xff, f);
        fputc((p >> 8) & 0xff, f);
        fputc(p & 0xff, f);
    ...
    /*
     * Check if composited output was all black but raw VRAM has data.
     * If so, re-dump as raw CI→grayscale (bypassing DID/XMAP/CMAP pipeline).
     */
    {
        uint32_t non_zero_rgbci = 0;
        for (int i = 0; i < PVREX3_VRAM_W * PVREX3_VRAM_H; i++) {
            if (s->vram_rgbci[i]) non_zero_rgbci++;
        }
        if (non_zero_rgbci > 1000) {
            /* VRAM has data — write raw CI grayscale dump */
            char rawpath[256];
            snprintf(rawpath, sizeof(rawpath), "%s.raw.ppm", path);
            FILE *rf = fopen(rawpath, "wb");
            if (rf) {
                fprintf(rf, "P6\n%d %d\n255\n", PVREX3_SCREEN_W, PVREX3_SCREEN_H);
                for (int ry = 0; ry < PVREX3_SCREEN_H; ry++) {
                    for (int rx = 0; rx < PVREX3_SCREEN_W; rx++) {
                        uint32_t ci = s->vram_rgbci[ry * PVREX3_VRAM_W + rx] & 0xff;
                        uint8_t v = ci;  /* 8-bit CI value as grayscale */
                        uint8_t rgb[3] = { v, v, v };
                        fwrite(rgb, 1, 3, rf);
                    }
                }
                fclose(rf);
                qemu_log("newport: fb-dump: raw CI grayscale → '%s' (%u non-zero VRAM pixels)\n",
                         rawpath, non_zero_rgbci);
            }
        }
    }
    ...
    /* Also log VRAM statistics for debugging */
    {
        ...
        qemu_log("newport: fb-dump: wrote %dx%d PPM to '%s'\n"
                 ...
                 PVREX3_SCREEN_W, PVREX3_SCREEN_H, path,
                 non_zero_rgbci, PVREX3_VRAM_W * PVREX3_VRAM_H,
                 non_zero_cidaux, PVREX3_VRAM_W * PVREX3_VRAM_H,
                 first_nz_idx, first_nz_val,
                 first_nz_idx % PVREX3_VRAM_W, first_nz_idx / PVREX3_VRAM_W,
...
static void pvrex3_invalidate(void *opaque)
{
...
static void pvrex3_update_display(void *opaque)
{
    ...
    for (y = 0; y < PVREX3_SCREEN_H; y++) {
        ...
        for (x = 0; x < PVREX3_SCREEN_W; x++) {
             ...
             *  1. Cursor pixel (handled in pvrex3_draw_cursor post-loop)
    ...
    dpy_gfx_update(s->con, 0, 0, PVREX3_SCREEN_W, PVREX3_SCREEN_H);
    s->display_dirty = false;
}

static const GraphicHwOps pvrex3_gfx_ops = {
    .invalidate = pvrex3_invalidate,
    .gfx_update = pvrex3_update_display,
};
...
static void sgi_pvrex3_reset(DeviceState *dev)
{
    SGIPVRex3State *s = SGI_PVREX3(dev);
    ...
    /*
     * CONFIG register encodes the board configuration.
     * For XL8 (8-bit Indy Newport):
     *   Bits indicate REX3 planes, display depth, optional hardware.
     *   gfxinit normally programs this, but the Xsgi DDX reads it
     *   during SETDISPLAYMODE to determine bitplane count.
     *   Non-zero value = "display pipeline present".
     *
     * Use 0x08 to indicate 8-bit configuration.
     */
    s->config = 0x08;
    ...
    /*
     * Pre-program VC2 DID (Display ID) tables so the compositing
     * pipeline works even without gfxinit programming display modes.
     *
     * The DID table maps each scanline to a DID value.
     * DID 0 → XMAP mode 0 → CI8 (8-bit color index through CMAP).
     *
     * VC2 DID structure:
     *   vc2_reg[DID_ENTRY] = SRAM address of frame DID table
     *   Frame table: array of 1024 line pointers (one per scanline)
     *   Line entry: (DID << 5) | (pixel_count - 1) pairs, terminated by 0
     *
     * Simplified: all scanlines point to the same line entry at SRAM 0x100
     *   Line entry at 0x100: DID=0, count=1279 → (0 << 5) | 1279 = 0x04FF
     *   Followed by 0x0000 (end-of-line)
     *
     *   Frame table at 0x200: 1024 entries, all pointing to 0x100
     */
    {
        /* Line DID entry at SRAM address 0x100 */
        s->vc2_ram[0x100] = (0 << 5) | (1280 - 1);  /* DID=0, 1280 pixels */
        s->vc2_ram[0x101] = 0;                        /* end-of-line */

        /* Frame table at 0x200: 1024 line pointers, all = 0x100 */
        for (int i = 0; i < 1024; i++) {
            s->vc2_ram[0x200 + i] = 0x0100;
        }

        /* Set VC2 DID_ENTRY register to point to frame table */
        s->vc2_reg[VC2_DID_ENTRY] = 0x0200;  /* DID frame table at SRAM 0x200 */

        /* Set VC2 timing registers for 1280x1024 mode.
         * These are read-only status registers on real Newport that reflect
         * the loaded timing table. The DDX reads these to verify SETDISPLAYMODE. */
        s->vc2_reg[VC2_VIDEO_ENTRY] = 0x0300;  /* Video timing table at SRAM 0x300 */
        s->vc2_reg[VC2_SCANLINE_LEN] = 1280;   /* Pixels per scanline */
        s->vc2_reg[VC2_VT_LINE_COUNT] = 1024;  /* Visible lines per frame */

        /* Enable display: set DC_CONTROL to enable DID processing */
        s->vc2_reg[VC2_DC_CONTROL] = VC2_DC_ENA_DISPLAY | VC2_DC_ENA_DIDS;
    }

    /* XMAP */
    s->xmap_config = 0x04;  /* CIDAUX bit = CI mode with AUX planes */
    s->xmap_revision = 1;
    memset(s->xmap_mode_table, 0, sizeof(s->xmap_mode_table));
    /*
     * XMAP mode table entry 0: CI8 (8-bit Color Index through CMAP0)
     * Mode register encoding (24 bits per entry):
     *   [23:20] = pixel size (0=CI, 1=RGB)
     *   [19:16] = CMAP select
     *   [15:10] = aux CID bits
     *   [9:5]   = overlay bits
     *   [4:0]   = underlay bits
     *
     * For CI8 mode: pixsize=0, cmap=0 → mode = 0x000000
     * (zero is correct for CI8 through CMAP0)
     */
    /*
     * CI8 mode with CMAP MSB = 0x80 → ci_msb = 0x80 << 5 = 0x1000
     * This selects CMAP1 (entries 4096+), which is where the Newport
     * DDX programs its color palette via DCB writes to CMAP1.
     */
    /*
     * CI8 mode with CMAP MSB = 0x90 → ci_msb = 0x90 << 5 = 0x1200 (4608)
     * The DDX programs CMAP palette starting at entry ~4609 (CMAP1+513).
     * This offset matches the standard SGI XL8 CMAP layout.
     *
     * ALL 32 entries must be initialized, not just [0]: the PROM/ng1
     * kernel driver initializes every mode register to 8-bit CI
     * (ng1_init.c: for(i=0;i<32;i++) xmap9SetModeReg(rex3,i,XM9_PIXSIZE_8,...))
     * and Xsgi never writes the mode table itself — its VC2 DID entries
     * reference arbitrary indices (e.g. the root uses 0xb) assuming the
     * kernel's invariant. With only [0] set, every non-zero index
     * resolved to mode 0x000000 → CMAP page 0 → black, which made
     * correctly-drawn windows (root fill among them) render as black.
     */
    for (int mi = 0; mi < 32; mi++) {
        s->xmap_mode_table[mi] = 0x000490;  /* CI8, CMAP base 0x1200 */
    }
    ...
    /* Pre-load CMAP0 with a basic 8-bit grayscale + SGI default colors */
    for (int i = 0; i < 256; i++) {
        /* Default SGI colormap: color 0 = black, 7 = white (for text) */
        uint8_t r = i, g = i, b = i;
        s->cmap0_palette[i] = (r << 16) | (g << 8) | b;
    }
    /* SGI standard: color 7 = white (used for default text) */
    s->cmap0_palette[7] = 0xFFFFFF;
    ...
    if (s->vram_rgbci) {
        memset(s->vram_rgbci, 0,
               PVREX3_VRAM_W * PVREX3_VRAM_H * sizeof(uint32_t));
    }
    if (s->vram_cidaux) {
...
static void sgi_pvrex3_realize(DeviceState *dev, Error **errp)
{
    SGIPVRex3State *s = SGI_PVREX3(dev);

    /* Allocate VRAM */
    s->vram_rgbci = g_malloc0(PVREX3_VRAM_W * PVREX3_VRAM_H *
                              sizeof(uint32_t));
    ...
    s->dcb_timeout_timer = timer_new_ns(QEMU_CLOCK_VIRTUAL,
                                         pvrex3_dcb_timeout, s);

    /* VBLANK timer — 60Hz vertical retrace interrupt */
    s->vblank_timer = timer_new_ns(QEMU_CLOCK_VIRTUAL,
                                    pvrex3_vblank_timer, s);
    s->vrint_deassert_timer = timer_new_ns(QEMU_CLOCK_VIRTUAL,
                                            pvrex3_vrint_deassert, s);
    timer_mod(s->vblank_timer,
              qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL) +
              NANOSECONDS_PER_SECOND / 60);

    /* Create display console */
    s->con = graphic_console_init(dev, 0, &pvrex3_gfx_ops, s);
    qemu_console_resize(s->con, PVREX3_SCREEN_W, PVREX3_SCREEN_H);

    s->display_dirty = true;
}

static void sgi_pvrex3_init(Object *obj)
{
    SGIPVRex3State *s = SGI_PVREX3(obj);
    SysBusDevice *sbd = SYS_BUS_DEVICE(obj);

    memory_region_init_io(&s->iomem, obj, &sgi_pvrex3_ops, s,
                          "sgi-pvrex3", REX3_REG_SIZE);
    sysbus_init_mmio(sbd, &s->iomem);

    /* IRQ output for vertical retrace → HEART ISR bit 21 */
    sysbus_init_irq(sbd, &s->irq);
}

static void sgi_pvrex3_finalize(Object *obj)
{
    SGIPVRex3State *s = SGI_PVREX3(obj);
...
static char *pvrex3_get_diag_cmap(Object *obj, Error **errp)
{
    SGIPVRex3State *s = SGI_PVREX3(obj);
...
static char *pvrex3_get_diag_xmap(Object *obj, Error **errp)
{
    SGIPVRex3State *s = SGI_PVREX3(obj);
...
static char *pvrex3_get_diag_vc2(Object *obj, Error **errp)
{
    SGIPVRex3State *s = SGI_PVREX3(obj);
...
static char *pvrex3_get_diag_rex3(Object *obj, Error **errp)
{
    SGIPVRex3State *s = SGI_PVREX3(obj);
    GString *buf = g_string_sized_new(2048);
...
static char *pvrex3_get_diag_dcb(Object *obj, Error **errp)
{
    SGIPVRex3State *s = SGI_PVREX3(obj);
...
static char *pvrex3_get_diag_all(Object *obj, Error **errp)
{
    SGIPVRex3State *s = SGI_PVREX3(obj);
    GString *buf = g_string_sized_new(1024);
    ...
    g_string_append_printf(buf,
        ...
        vram_nz, cidaux_nz, PVREX3_VRAM_W * PVREX3_VRAM_H);
    if (vram_nz) {
        g_string_append_printf(buf,
            "VRAM: first_nz idx=%u val=0x%08x (x=%u,y=%u)\n",
            vram_first_idx, vram_first_val,
            vram_first_idx % PVREX3_VRAM_W,
            vram_first_idx / PVREX3_VRAM_W);
...
static char *pvrex3_get_fb_dump(Object *obj, Error **errp)
{
    return g_strdup("");
}

static void pvrex3_set_fb_dump(Object *obj, const char *value, Error **errp)
{
    SGIPVRex3State *s = SGI_PVREX3(obj);
...
#define PVREX3_VRAM_BYTES \
    (PVREX3_VRAM_W * PVREX3_VRAM_H * sizeof(uint32_t))

static const VMStateDescription vmstate_sgi_pvrex3 = {
    .name = "sgi-pvrex3",
    .version_id = 5,
    .minimum_version_id = 5,
    .post_load = sgi_pvrex3_post_load,
    .fields = (const VMStateField[]) {
        /* REX3 drawing registers */
        VMSTATE_UINT32(drawmode0, SGIPVRex3State),
        VMSTATE_UINT32(drawmode1, SGIPVRex3State),
        VMSTATE_UINT32(ls_mode, SGIPVRex3State),
        VMSTATE_UINT32(ls_pattern, SGIPVRex3State),
        VMSTATE_UINT32(ls_pattern_saved, SGIPVRex3State),
        VMSTATE_UINT32(z_pattern, SGIPVRex3State),
        VMSTATE_UINT32(color_back, SGIPVRex3State),
        VMSTATE_UINT32(color_vram, SGIPVRex3State),
        VMSTATE_UINT32(alpha_ref, SGIPVRex3State),
        VMSTATE_UINT32_ARRAY(smask_x, SGIPVRex3State, 5),
        VMSTATE_UINT32_ARRAY(smask_y, SGIPVRex3State, 5),
        VMSTATE_UINT32(setup, SGIPVRex3State),
        VMSTATE_UINT32(step_z, SGIPVRex3State),

        /* Coordinate registers */
        VMSTATE_INT32(x_start, SGIPVRex3State),
        VMSTATE_INT32(y_start, SGIPVRex3State),
        VMSTATE_INT32(x_end, SGIPVRex3State),
        VMSTATE_INT32(y_end, SGIPVRex3State),
        VMSTATE_INT32(x_save, SGIPVRex3State),
        VMSTATE_UINT32(xy_move, SGIPVRex3State),
        VMSTATE_UINT32(bres_d, SGIPVRex3State),
        VMSTATE_UINT32(bres_s1, SGIPVRex3State),
        VMSTATE_UINT32(bres_octant_inc1, SGIPVRex3State),
        VMSTATE_UINT32(bres_round_inc2, SGIPVRex3State),
        VMSTATE_UINT32(bres_e1, SGIPVRex3State),
        VMSTATE_UINT32(bres_s2, SGIPVRex3State),
        VMSTATE_UINT32(a_weight0, SGIPVRex3State),
        VMSTATE_UINT32(a_weight1, SGIPVRex3State),
        VMSTATE_UINT32(x_start_f, SGIPVRex3State),
        VMSTATE_UINT32(y_start_f, SGIPVRex3State),
        VMSTATE_UINT32(x_end_f, SGIPVRex3State),
        VMSTATE_UINT32(y_end_f, SGIPVRex3State),
        VMSTATE_INT32(x_start_i, SGIPVRex3State),
        VMSTATE_UINT32(xy_start_i, SGIPVRex3State),
        VMSTATE_UINT32(xy_end_i, SGIPVRex3State),
        VMSTATE_UINT32(x_start_end_i, SGIPVRex3State),

        /* Integer coordinate state */
        VMSTATE_INT16(iter_x, SGIPVRex3State),
        VMSTATE_INT16(iter_y, SGIPVRex3State),
        VMSTATE_INT16(x_start_int, SGIPVRex3State),
        VMSTATE_INT16(y_start_int, SGIPVRex3State),
        VMSTATE_INT16(x_end_int, SGIPVRex3State),
        VMSTATE_INT16(y_end_int, SGIPVRex3State),
        VMSTATE_INT16(x_save_int, SGIPVRex3State),

        /* Color registers */
        VMSTATE_UINT32(write_mask, SGIPVRex3State),
        VMSTATE_UINT32(color_i, SGIPVRex3State),
        VMSTATE_UINT32(zero_overflow, SGIPVRex3State),
        VMSTATE_UINT32(color_red, SGIPVRex3State),
        VMSTATE_UINT32(color_alpha, SGIPVRex3State),
        VMSTATE_UINT32(color_green, SGIPVRex3State),
        VMSTATE_UINT32(color_blue, SGIPVRex3State),
        VMSTATE_UINT32(curr_color_red, SGIPVRex3State),
        VMSTATE_UINT32(curr_color_alpha, SGIPVRex3State),
        VMSTATE_UINT32(curr_color_green, SGIPVRex3State),
        VMSTATE_UINT32(curr_color_blue, SGIPVRex3State),
        VMSTATE_INT32(slope_red, SGIPVRex3State),
        VMSTATE_INT32(slope_alpha, SGIPVRex3State),
        VMSTATE_INT32(slope_green, SGIPVRex3State),
        VMSTATE_INT32(slope_blue, SGIPVRex3State),
        VMSTATE_UINT64(host_dataport, SGIPVRex3State),
        VMSTATE_UINT32(host_shift, SGIPVRex3State),
        VMSTATE_UINT32(global_mask, SGIPVRex3State),

        /* DCB */
        VMSTATE_UINT32(dcb_mode, SGIPVRex3State),
        VMSTATE_UINT32(dcb_data_msw, SGIPVRex3State),
        VMSTATE_UINT32(dcb_data_lsw, SGIPVRex3State),

        /* Screenmask and clipping */
        VMSTATE_UINT32(top_scanline, SGIPVRex3State),
        VMSTATE_UINT32(xy_window, SGIPVRex3State),
        VMSTATE_UINT32(clip_mode, SGIPVRex3State),

        /* Config and status */
        VMSTATE_UINT32(config, SGIPVRex3State),
        VMSTATE_UINT32(status, SGIPVRex3State),

        /* VC2 */
        VMSTATE_UINT32(vc2_ram_addr, SGIPVRex3State),
        VMSTATE_UINT32(vc2_reg_idx, SGIPVRex3State),
        VMSTATE_UINT32(vc2_reg_data, SGIPVRex3State),
        VMSTATE_UINT16_ARRAY(vc2_ram, SGIPVRex3State, 32768),
        VMSTATE_UINT16_ARRAY(vc2_reg, SGIPVRex3State, 32),

        /* XMAP9 */
        VMSTATE_UINT32(xmap_config, SGIPVRex3State),
        VMSTATE_UINT32(xmap_revision, SGIPVRex3State),
        VMSTATE_UINT8(xmap_cursor_cmap, SGIPVRex3State),
        VMSTATE_UINT8(xmap_popup_cmap, SGIPVRex3State),
        VMSTATE_UINT8(xmap_mode_table_idx, SGIPVRex3State),
        VMSTATE_UINT32_ARRAY(xmap_mode_table, SGIPVRex3State, 32),

        /* CMAP */
        VMSTATE_UINT32(cmap_revision, SGIPVRex3State),
        VMSTATE_UINT16(cmap_palette_idx, SGIPVRex3State),
        VMSTATE_UINT32_ARRAY(cmap0_palette, SGIPVRex3State, 8192),

        /* RAMDAC */
        VMSTATE_UINT8(ramdac_lut_index, SGIPVRex3State),
        VMSTATE_UINT32_ARRAY(ramdac_lut_r, SGIPVRex3State, 256),
        VMSTATE_UINT32_ARRAY(ramdac_lut_g, SGIPVRex3State, 256),
        VMSTATE_UINT32_ARRAY(ramdac_lut_b, SGIPVRex3State, 256),

        /* VRAM (heap-allocated, fixed size) */
        VMSTATE_BUFFER_POINTER_UNSAFE(vram_rgbci, SGIPVRex3State, 0,
                                      PVREX3_VRAM_BYTES),
        VMSTATE_BUFFER_POINTER_UNSAFE(vram_cidaux, SGIPVRex3State, 0,
                                      PVREX3_VRAM_BYTES),

        /* Display dirty flag */
        VMSTATE_BOOL(display_dirty, SGIPVRex3State),
...
static void sgi_pvrex3_class_init(ObjectClass *klass, const void *data)
{
    ...
    dc->realize = sgi_pvrex3_realize;
    device_class_set_legacy_reset(dc, sgi_pvrex3_reset);
    dc->vmsd = &vmstate_sgi_pvrex3;
    ...
    object_class_property_add_str(klass, "diag-cmap",
                                  pvrex3_get_diag_cmap, NULL);
    object_class_property_add_str(klass, "diag-xmap",
                                  pvrex3_get_diag_xmap, NULL);
    object_class_property_add_str(klass, "diag-vc2",
                                  pvrex3_get_diag_vc2, NULL);
    object_class_property_add_str(klass, "diag-rex3",
                                  pvrex3_get_diag_rex3, NULL);
    object_class_property_add_str(klass, "diag-dcb",
                                  pvrex3_get_diag_dcb, NULL);
    object_class_property_add_str(klass, "diag-all",
                                  pvrex3_get_diag_all, NULL);
}

static const TypeInfo sgi_pvrex3_info = {
    .name              = "sgi-pvrex3",
    .parent            = TYPE_SYS_BUS_DEVICE,
    .instance_size     = sizeof(SGIPVRex3State),
    .instance_init     = sgi_pvrex3_init,
    .instance_finalize = sgi_pvrex3_finalize,
    .class_init        = sgi_pvrex3_class_init,
};

static void sgi_pvrex3_register_types(void)
{
    type_register_static(&sgi_pvrex3_info);
}

type_init(sgi_pvrex3_register_types)
