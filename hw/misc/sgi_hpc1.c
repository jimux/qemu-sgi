...
static uint64_t sgi_hpc1_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    if (addr >= HPC1_ENET_XCOUNT && addr < 0x60) {
        switch (addr & ~3ULL) {
        case HPC1_ENET_XCOUNT:   return s->enet_xcount;
        case HPC1_ENET_CXBP:     return s->enet_cxbp;
        case HPC1_ENET_NXBDP:    return s->enet_nxbdp;
        case HPC1_ENET_XBC:      return s->enet_xbc;
        case HPC1_ENET_CXBDP:    return s->enet_cxbdp;
        case HPC1_ENET_CPFXBDP:  return s->enet_cpfxbdp;
        case HPC1_ENET_PPFXBDP:  return s->enet_ppfxbdp;
        case HPC1_ENET_INTDELAY: return s->enet_intdelay;
        case HPC1_ENET_TRSTAT:   return s->enet_trstat;
        case HPC1_ENET_RCVSTAT:  return s->enet_rcvstat;
        case HPC1_ENET_CTL:      return s->enet_ctl;
        case HPC1_ENET_RBC:      return s->enet_rbc;
        case HPC1_ENET_CRBP:     return s->enet_crbp;
        case HPC1_ENET_NRBDP:    return s->enet_nrbdp;
        case HPC1_ENET_CRBDP:    return s->enet_crbdp;
        default:                 return 0;
        }
    }
                           ...
                           unsigned size)
{
    ...
    if (addr >= HPC1_ENET_XCOUNT && addr < 0x60) {
        ...
        case HPC1_ENET_NRBDP:
            s->enet_nrbdp = value;
            s->enet_crbdp = value;      /* driver sets both when arming RX */
            break;
        case HPC1_ENET_INTDELAY:
            s->enet_intdelay = value;
            break;
        ...
        case HPC1_ENET_CTL:
            ...
            break;
        default:
            break;   /* other enet regs are read-only to the guest */
        }
        return;
    }
