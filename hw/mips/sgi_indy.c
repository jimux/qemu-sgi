 ...
 *   - Indigo (IP20): R4000, MC/HPC1/INT2 (stub)
 *
 ...
 * and INT2 instead of IOC2. Those devices are not yet implemented, so
 * IP20 is a stub that will fail on peripheral detection.
 *
 * References:
...
static uint64_t gio_empty_slot_read(void *opaque, hwaddr offset, unsigned size)
{
    ...
    qemu_log_mask(LOG_UNIMP, "%s: empty slot read (size %d, offset 0x%06"
                  HWADDR_PRIx ") -> 0xffffffff\n", s->name, size, offset);
    return 0xffffffffffffffffULL;  /* All bits set = no device */
}

static void gio_empty_slot_write(void *opaque, hwaddr offset,
                                  uint64_t value, unsigned size)
{
    GIOEmptySlotState *s = opaque;

    qemu_log_mask(LOG_UNIMP, "%s: empty slot write (size %d, offset 0x%06"
...
#define MIPS_AND(rd, rs, rt) (0x00000024 | ((rs) << 21) | ((rt) << 16) | \
                              ((rd) << 11))
#define MIPS_LUI(rt, imm)   (0x3C000000 | ((rt) << 16) | ((imm) & 0xFFFF))
#define MIPS_ORI(rt, rs, im) (0x34000000 | ((rs) << 21) | ((rt) << 16) | \
                              ((im) & 0xFFFF))
...
#define CP0_STATUS 12
    ...
    trampoline[i++] = cpu_to_be32(
        MIPS_LUI(A2, (MIPS_K0BASE + ARCS_ENVIRON_PHYS) >> 16));
    trampoline[i++] = cpu_to_be32(
        MIPS_ORI(A2, A2, (MIPS_K0BASE + ARCS_ENVIRON_PHYS) & 0xFFFF));
    ...
    rom_add_blob_fixed("kernel-trampoline", trampoline, i * 4,
                       SGI_PROM_BASE);
...
{
    switch (model) {
    case SGI_IP20: return "ip20prom.bin";
    case SGI_IP22: return "ip22prom.bin";
    case SGI_IP24: return "ip24prom.bin";
    case SGI_IP26: return "ip26prom.bin";
    case SGI_IP28: return "ip28prom.bin";
    default:       return "ip24prom.bin";
    }
}
...
{
    switch (model) {
    case SGI_IP24: return "sgi_indy_nvram.bin";
    case SGI_IP22: return "sgi_indigo2_nvram.bin";
    case SGI_IP28: return "sgi_indigo2_r10k_nvram.bin";
    case SGI_IP26: return "sgi_indigo2_r8k_nvram.bin";
    case SGI_IP20: return "sgi_indigo_nvram.bin";
    default:       return "sgi_nvram.bin";
    }
}
...
{
    switch (model) {
    case SGI_IP20: return "IP20";
    case SGI_IP22: return "IP22";
    case SGI_IP24: return "IP24";
    case SGI_IP26: return "IP26";
    case SGI_IP28: return "IP28";
    default:       return "IP2x";
    }
}
...
static void sgi_ip2x_init(MachineState *machine, enum sgi_ip2x_model model)
{
    ...
    char *filename;
    int bios_size;
    bool is_fullhouse = (model == SGI_IP22 || model == SGI_IP26 ||
                         model == SGI_IP28);
    ...
    /* Warn about IP20 limitations */
    if (model == SGI_IP20) {
        warn_report("IP20 (Indigo) uses HPC1/INT2 which are not implemented. "
                    "The PROM will fail on peripheral detection. "
                    "This machine type is a stub for PROM analysis.");
    }
    ...
    memory_region_init_rom(prom, NULL, "sgi.prom", SGI_PROM_SIZE,
                           &error_fatal);
    ...
    if (filename) {
        bios_size = load_image_targphys(filename, SGI_PROM_BASE,
                                        SGI_PROM_SIZE, NULL);
        g_free(filename);
        if (bios_size < 0) {
    ...
    /* Set per-machine NVRAM backing file */
     ...
     */
    qdev_connect_gpio_out_named(hpc3_dev, "cpu-irq", 0,
                                cpu->env.irq[2]);  /* Local0 -> IP2 */
    qdev_connect_gpio_out_named(hpc3_dev, "cpu-irq", 1,
                                cpu->env.irq[3]);  /* Local1 -> IP3 */
    qdev_connect_gpio_out_named(hpc3_dev, "timer-irq", 0,
                                cpu->env.irq[4]);  /* Timer0 -> IP4 */
    qdev_connect_gpio_out_named(hpc3_dev, "timer-irq", 1,
                                cpu->env.irq[5]);  /* Timer1 -> IP5 */
     ...
     */
    {
        ...
        create_gio_empty_slot(system_memory, "gio-gfx-low",
        ...
        /* Wire Newport VRINT → HPC3 INT3_LOCAL1 bit 7 (GIO2/Retrace) */
        sysbus_connect_irq(SYS_BUS_DEVICE(newport_dev), 0,
                           qdev_get_gpio_in_named(hpc3_dev,
                                                  "gio-retrace", 0));
    ...
    create_gio_empty_slot(system_memory, "gio-exp0", SGI_GIO_EXP0_BASE,
                          2 * MiB);
    create_gio_empty_slot(system_memory, "gio-exp1", SGI_GIO_EXP1_BASE,
                          4 * MiB);
     ...
     */
    create_unimplemented_device("low-mem-probe", SGI_RAM_LOW_BASE,
                                SGI_RAM_MAX);
    create_unimplemented_device("high-mem-probe", SGI_RAM_HIGH_BASE,
                                SGI_RAM_MAX);
     ...
     * bus errors.
     */
    create_unimplemented_device("extended-mem-probe0",
                                0x18000000, 0x07000000);  /* 0x18M–0x1EFFFFFF */
    create_unimplemented_device("extended-mem-probe1",
                                0x30000000, 0x50000000);  /* 0x30M–0x7FFFFFFF */
    ...
    if (machine->kernel_filename) {
        ...
        qemu_log("SGI: Kernel entry: 0x%016" PRIx64
                 ", highest addr: 0x%016" PRIx64
                 " (phys 0x%08" PRIx64 ")\n",
...
static void sgi_indigo_class_init(ObjectClass *oc, const void *data)
{
    ...
    mc->desc = "SGI Indigo (IP20) [stub - HPC1/INT2 not implemented]";
