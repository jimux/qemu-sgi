 ...
 *
 * This implements a paravirtual REX3 drawing engine with the same register
 * interface as the real Newport (XL) graphics controller, but without
 * the GIO bus slot mapping. Intended for use with the IP54 paravirtual
 * machine type.
 *
...
#ifndef HW_DISPLAY_SGI_PVREX3_H
#define HW_DISPLAY_SGI_PVREX3_H
...
#define PVREX3_VRAM_W      1344    /* 1280 + 64 */
#define PVREX3_VRAM_H      1088    /* 1024 + 64 */
#define PVREX3_SCREEN_W    1280
#define PVREX3_SCREEN_H    1024

#define TYPE_SGI_PVREX3 "sgi-pvrex3"
OBJECT_DECLARE_SIMPLE_TYPE(SGIPVRex3State, SGI_PVREX3)

/*
 * REX3 Register offsets
 *
 * REX3 registers can be accessed as 32-bit or 64-bit; the register layout
 * pairs registers at offsets N+4 and N+0 into 64-bit words.
 *
 * Offsets listed are relative to REX3 base.
 */
...
/*
 * GL window clip channel (VirGL roadmap Step 4) — a private register block in the
 * high, REX3-unused part of the 8KB window (0x1C00+). The kernel pvfb_gf_ValidateClip
 * hook writes the GL window geometry + visible clip pieces here; pvrex3 latches them on
 * CLIP_COMMIT and the compositor positions/clips the GL overlay accordingly.
 *   sequence: write WID/XORG/YORG/XSIZE/YSIZE/OBSCURED/NUMPIECES, then for each piece
 *   write PIECE_X/Y/W/H + PIECE_PUSH<index>, finally CLIP_COMMIT.
 */
...
#define REX3_CLIP_PIECE_PUSH    0x1c30  /* write piece index -> store staged piece */
#define REX3_CLIP_COMMIT        0x1c3c  /* write 1 -> latch clip record */

/* REX3 memory region size */
...
struct SGIPVRex3State {
    ...
    /*
     * Accelerated-graphics GL overlay (VirGL roadmap Step 3): a host-rendered RGBA
     * frame composited on top of the desktop (under the cursor) at a clipped window
     * rect. Set by the sgi_pvgl compositor device / QOM test hook. Compositing into
     * the final DisplaySurface avoids the VRAM CI/palette format entirely.
     */
    bool      gl_overlay_active;
    int32_t   gl_overlay_x, gl_overlay_y, gl_overlay_w, gl_overlay_h;
    uint32_t *gl_overlay_rgba;      /* gl_overlay_w * gl_overlay_h, 0x00RRGGBB */

    /*
     * Live GL frame channel (socket): the host renderer streams PVGL frames
     * (20-byte header [magic "PVGL", int32 x,y,w,h LE] + w*h*4 RGBA) to a socket
     * pvrex3 listens on (set via the "gl-listen" QOM property). Reassembled in the
     * QEMU main loop and composited live.
     */
    int       gl_listen_fd;         /* -1 if not listening */
    int       gl_conn_fd;           /* -1 if no renderer connected */
    void     *gl_rxbuf;             /* GByteArray * accumulator */

    /*
     * Window clip record (VirGL roadmap Step 4): the kernel pvfb_gf_ValidateClip
     * hook writes the GL window's screen geometry + visible clip pieces here via the
     * clip-channel MMIO block (REX3 offset 0x1C00+). The compositor positions the GL
     * overlay at (clip_xorg, clip_yorg) for the matching WID and blits only inside the
     * visible pieces (occlusion), skipping entirely when obscured. clip_valid gates it;
     * with no clip record the overlay falls back to its frame-channel x,y (unclipped).
     */
#define PVREX3_CLIP_MAX_PIECES 32
    bool      clip_valid;
    int32_t   clip_wid;
    int32_t   clip_xorg, clip_yorg, clip_xsize, clip_ysize;
    int32_t   clip_obscured;
    int32_t   clip_numpieces;
    int32_t   clip_pieces[PVREX3_CLIP_MAX_PIECES * 4];  /* x,y,w,h per piece */
    /* staging for the incremental piece-write protocol */
    int32_t   clip_stage_idx;
    int32_t   clip_stage[4];
