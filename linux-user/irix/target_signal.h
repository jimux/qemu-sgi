/*
 * IRIX target signal definitions for linux-user.
 *
 * Ported from qemu-irix (Kai-Uwe Bloem <derkub@gmail.com>;
 * n64decomp/qemu-irix), GPLv2. In qemu-irix these lived in syscall_defs.h
 * (the shared MIPS/Solaris signal block); in QEMU 10.x the per-ABI signal
 * numbers live in target_signal.h. IRIX shares the MIPS signal numbers.
 */
...
/* IRIX/SVR4 sigaction flags (from qemu-irix syscall_defs.h IRIX||SOLARIS) */
...
/* get_sp_from_cpustate() is provided by mips/target_cpu.h */
...
/* IRIX has traditional (non-rt) signals dispatched via setup_frame(). */
#define TARGET_ARCH_HAS_SETUP_FRAME

#endif /* IRIX_TARGET_SIGNAL_H */
